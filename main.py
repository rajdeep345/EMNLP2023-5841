...
from preprocessor_linearized import ASTE_Dataset
from utils_linearized import correct_spaces, get_f1_for_trainer
...
class T5FineTuner(pl.LightningModule):
	...
	def _generate(self, batch):
		...
		for i in range(len(outs)):
			...
			dec = tokenizer.decode(outs[i], skip_special_tokens=True)
			labels = np.where(batch["target_ids"][i].cpu().numpy() != -100, batch["target_ids"][i].cpu().numpy(), tokenizer.pad_token_id)
			target = tokenizer.decode(torch.tensor(labels), skip_special_tokens=True)
